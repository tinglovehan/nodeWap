...
$(function () {
    ...
    $('.repeat-li ').click(function () {
        $(this).siblings('.repast-imgcnt').show();
        $('.mask').show();
    })
    $('.repast-imgcnt').find('.close-ticket').click(function () {
        $(this).parent().hide();
        $('.mask').hide();
    })
    $('.mask').click(function () {
...
function listDom(list, begin, end) {
    ...
    while (len--) {
        ...
        dom += '<li>' +
            '<div class="pro-info">' +
            '<div class="pro-img showDetail" '+'data-modelCode=' +list[len].modelCode +  ' data-module= "hotel">' +
            '<img src="' + imgurl + '" alt=""/>' +'</div>'
        if (ratelength > 0) {
            $.each(ratecode, function (i) {
                dom +='<div class="hotel-info hotel-info2">'+
                    '<h4>' + ratecode[i].aliasName + '</h4>' +
                    '<p class="pro-info-explian"><span>' + bedType(ratecode[i].bedType) + '</span><span>' + ratecode[i].buildingArea + 'm²</span>'+
                    '<div class="pro-price c-base pro-mar">' +
                    '<span class="price"><em>￥</em><strong>' + ratecode[i].currentPrice + '</strong></span>' +
                    '<span class="original-price"><em>门市价： ￥' + ratecode[i].priceShow + '</em></span>' +
                    '</div></div>' +
                    '<div class="pro-price">' +
    ...
    _showDetail.unbind('click').click(function (e) {
        ...
        if(module=='hotel'){
            $.get('/detail/picture', {
                ...
                dom+='</ul></div>'
                if( _this.parents('li').find('.ticket-layer').find('.ht-pic').find('.flexslider').length<=0){
