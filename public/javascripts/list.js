...
$(function () {
    ...
    var dropBox = $(".drop-box"),
        ...
        classifyNo = $('.list-tab').find('li').eq(0).data('filter'),

        pageSize = 6, // 每页数据条数
        // filterObj = module === 'hotel'
        //     ? { currPage: 1, pageSize: pageSize, beginDate: beginDate, endDate: endDate ,classifyId:classifyNo}
        //     : { currPage: 1, pageSize: pageSize,classifyId:classifyNo }; // 定义一个对象用于存储筛选条件,默认筛选为翻页第一页
    ...
    if(h_type=='0'){
        $('#search-h').removeClass('search-box').addClass('search-box2');
    }
    ...
    touch.on("#searchBar button", "tap", function () {
        ...
        if(module=='strategy') {
            filterObj.parentCode = parentCode;
        }
        //unLockDropload();
        filterFn(dropload, 1);
    });
    ...
    touch.on(".list-tab li", "tap", function () {
        if(module=='hotel'){
            h_type=$(this).data('type');
        }
        if($(this).data('type')=='0'){
            $(this).parent().parent().siblings('.drop-box ').find('ul').removeClass('list-main').addClass('list-Smain');
            $(this).parent().parent().siblings('#search-h ').removeClass('search-box').addClass('search-box2');
        }else{
        ...
        var classifyNo = $(this).data('filter');
        ...
        if (module === 'order') {
            if (list[len].orderType !== 'shop') {
                dom += '<div class="myorder-item">' +
                    ...
                    '<h3 class="myorder-item-title">' + list[len].orderInfo + '</h3>'+'<p>' + (list[len].orderStatus === "2" ? '已使用' : '未使用') + '( ' + list[len].startTime.substring(0, 10) + ' 使用)</p>' +
            ...
            switch (module) {
                ...
                case 'hotel':
                    if(h_type=='0'){
                        // dom +='<div class="Ty-htbg"><h2>'+eN(list[len].aliasName)+'</h2></div>'
                    }else{
                        list[len].priceShow ? _price = '<span class="price fr"><em>￥</em><strong>' + (list[len].priceShow).toFixed(2) + '</strong>起</span>' : _price = '';
                        dom += '<div class="page-list-info">' +
                               '<h3 class="page-list-title"><div class="list-title-box">' + eN(list[len].aliasName) + '</div>'+
                               '<p class="page-list-explian">'+ eN(list[len].areaName)+ '</p>' +
                              tag + '</h3>' +
                            '<p class="page-list-explian"><label class="fl">评分：'+ list[len].avgScore+'&nbsp评论数：'+list[len].countComment + '</label>'+ _price + '</p>' +
                            '</div></a></li>';
                    }
                ...
                case 'car':
                    dom += '<div class="page-list-info">' +
                        ...
                        '<p class="page-list-explian">' + '<span class="c-base">' + (list[len].salesNum || 0) + '</span>人已购买'  +
                ...
                case 'guide':
                    ...
                    dom += '<div class="page-list-info"><h3 class="goods-list-title"><div class="list-title-box">' + eN(list[len].aliasName) + '</div></h3>' +
                        '<p class="goods-list-article">' + eN(list[len].subtitle) + '</p>' +
                        '<p class="goods-list-explian">' +
                        _price +
                        '<span class="fr">月销量：' + (eN(list[len].salesNum) || 0) + '</span>' +
