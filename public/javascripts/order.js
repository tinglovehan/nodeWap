  ...
  $ord.formBtn.click(subForm);
    ...
    var initDom = function (date, tag) {
        ...
        if (_price) {
            ...
            $ord.price.text('暂无价格');
            $ord.formBtn.addClass('background-gray');
        }
    ...
    // 加入购物车和立即预定
    function subForm() {
        if( $ord.formBtn.hasClass('background-gray')){
            return false;
        }
        if (module === 'shop' && !goodsWayType) {
            var _a1=$('select[name=address1]');
            var _a3 = $('select[name=address3]');
            if(_a1.val()!='344,香港'&& _a1.val()!='446,澳门'&& _a1.val()!='TW,台湾' ){
                if (!_a3.val()) {
                ...
                $.post(_url)
                    .success(function (data) {
                        ...
                        if (datas.status === 200 && _b) {
                            ...
                            $ord.formBtn.removeClass('background-gray');
